...
module SlipstreamClient
  # Represents a practice site or organisation in Slipstream
  class PracticeEntity
    # A URL-safe unique identifier for an entity (See [Sqids](https://sqids.org))
    attr_accessor :slug

    # What you call this entity within your PMS
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `SlipstreamClient::PracticeEntity` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `SlipstreamClient::PracticeEntity`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if !@slug.nil? && @slug.to_s.length > 20
        invalid_properties.push('invalid value for "slug", the character length must be smaller than or equal to 20.')
      end
    ...
    def valid?
      ...
      return false if !@slug.nil? && @slug.to_s.length > 20
    ...
    def slug=(slug)
      ...
      if slug.to_s.length > 20
        fail ArgumentError, 'invalid value for "slug", the character length must be smaller than or equal to 20.'
      end
    ...
    def hash
      [slug, pms_specific_id, entity_type, practice_management_software].hash
    end
