...
module SlipstreamClient
  ...
  class ShortenedLink
    ...
    # How long after the last use or change the link will be deleted
    attr_accessor :sliding_expiration

    # A datetimeoffset string of when the link will expire and redirect to a 404 page with the return url
    ...
    # The url to redirect to after the link has expired
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `SlipstreamClient::ShortenedLink` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `SlipstreamClient::ShortenedLink`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if @link_slug.to_s.length > 20
        invalid_properties.push('invalid value for "link_slug", the character length must be smaller than or equal to 20.')
      end
    ...
    def valid?
      ...
      return false if @link_slug.to_s.length > 20
    ...
    def link_slug=(link_slug)
      ...
      if link_slug.to_s.length > 20
        fail ArgumentError, 'invalid value for "link_slug", the character length must be smaller than or equal to 20.'
      end
    ...
    def hash
      [link_slug, primary_short_link, full_link, click_tracking_enabled, sliding_expiration, expiration, _alias, primary_alias_link, visits, return_url].hash
    end
