...
module SlipstreamClient
  # Practitioner data for upload
  class PractitionerUpload
    # The PMS-specific identifier for the practitioner
    attr_accessor :external_id

    # The display name of the practitioner
    attr_accessor :name

    # The practitioner's speciality or qualification
    attr_accessor :speciality

    # URL to the practitioner's photo
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `SlipstreamClient::PractitionerUpload` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `SlipstreamClient::PractitionerUpload`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if !@speciality.nil? && @speciality.to_s.length > 100
        invalid_properties.push('invalid value for "speciality", the character length must be smaller than or equal to 100.')
      end

      if !@photo_url.nil? && @photo_url.to_s.length > 500
        invalid_properties.push('invalid value for "photo_url", the character length must be smaller than or equal to 500.')
      end
    ...
    def valid?
      ...
      return false if !@speciality.nil? && @speciality.to_s.length > 100
      return false if !@photo_url.nil? && @photo_url.to_s.length > 500
    ...
    # Custom attribute writer method with validation
    # @param [Object] speciality Value to be assigned
    def speciality=(speciality)
      if !speciality.nil? && speciality.to_s.length > 100
        fail ArgumentError, 'invalid value for "speciality", the character length must be smaller than or equal to 100.'
      end
    ...
    def photo_url=(photo_url)
      if !photo_url.nil? && photo_url.to_s.length > 500
        fail ArgumentError, 'invalid value for "photo_url", the character length must be smaller than or equal to 500.'
      end
    ...
    def hash
      [external_id, name, speciality, photo_url, provider_type].hash
    end
