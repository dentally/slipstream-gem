...
module SlipstreamClient
  # The request body when updating online booking site details. Omitted fields will not be updated.
  class UpdateOnlineBookingSiteDetailsRequest
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `SlipstreamClient::UpdateOnlineBookingSiteDetailsRequest` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `SlipstreamClient::UpdateOnlineBookingSiteDetailsRequest`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def hash
      [is_active, client_identifier, client_subscription_id, client_unique_id, name, use_provider_availability, use_direct_booking, use_new_cx_api_version].hash
    end
