...
module SlipstreamClient
  # The details of an API account, including the API key
  class ApiAccountDetailsWithKey
    ...
    # The API key associated with the account. Only returned on creation or key rotation.
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `SlipstreamClient::ApiAccountDetailsWithKey` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `SlipstreamClient::ApiAccountDetailsWithKey`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if !@api_key.nil? && @api_key.to_s.length > 70
        invalid_properties.push('invalid value for "api_key", the character length must be smaller than or equal to 70.')
      end
    ...
    def valid?
      ...
      return false if !@api_key.nil? && @api_key.to_s.length > 70
      true
    end
    ...
    def api_key=(api_key)
      if !api_key.nil? && api_key.to_s.length > 70
        fail ArgumentError, 'invalid value for "api_key", the character length must be smaller than or equal to 70.'
      end
    ...
    def hash
      [identifier, display_name, email, website, phone, organisation_slug, quota, api_key].hash
    end
