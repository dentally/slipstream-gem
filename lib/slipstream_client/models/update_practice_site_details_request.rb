...
module SlipstreamClient
  # The request body when updating practice details. Omitted fields will not be updated.
  class UpdatePracticeSiteDetailsRequest
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `SlipstreamClient::UpdatePracticeSiteDetailsRequest` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `SlipstreamClient::UpdatePracticeSiteDetailsRequest`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def hash
      [pms_specific_site_id, name, address, postal_address, phone_number, email_address, application_version, currency, website_url, logo_url, privacy_policy_url, time_zone, google_tags].hash
    end
