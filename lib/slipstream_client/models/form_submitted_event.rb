...
module SlipstreamClient
  ...
  class FormSubmittedEvent
    # The metadata containing the users responses to the form
    attr_accessor :current_state

    # The URL to the response file rendered from the submitted form
    attr_accessor :response_file_url

    # The date the form was submitted
    attr_accessor :submitted_date

    # The slug that uniquely identifies the form
    attr_accessor :id

    # The slug that uniquely identifies the form group
    attr_accessor :group_id

    # The display name of the form group
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `SlipstreamClient::FormSubmittedEvent` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `SlipstreamClient::FormSubmittedEvent`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def valid?
      ...
      return false if !@group_id.nil? && @group_id !~ Regexp.new(/^_[a-zA-Z0-9]{3,}$/)
      return false if !@user_id.nil? && @user_id !~ Regexp.new(/^_[a-zA-Z0-9]{3,}$/)
      true
    end
    ...
    def hash
      [current_state, response_file_url, submitted_date, id, group_id, group_display_name, display_name, type, type_id, user_id, user_display_name, user_first_name, user_last_name, user_email_address, user_phone].hash
    end
