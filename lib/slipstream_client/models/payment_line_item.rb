...
module SlipstreamClient
  # The details of a payment line item
  class PaymentLineItem
    ...
    # The number of items. Default is 1.
    ...
    def self.openapi_types
      {
        ...
        :'tax_behaviour' => :'PaymentTaxBehaviour'
      }
    end
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `SlipstreamClient::PaymentLineItem` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `SlipstreamClient::PaymentLineItem`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'tax_behaviour')
        self.tax_behaviour = attributes[:'tax_behaviour']
      end
    end
    ...
    def hash
      [price, quantity, product_name, product_description, tax_behaviour].hash
    end
